...
// //   return (
// //     <article className="bg-white/90 card shadow-xl p-4 flex flex-col h-[500px] rounded-2xl border border-[#F9461C]/10 hover:shadow-2xl hover:scale-[1.03] transition-all duration-300 relative overflow-hidden group">
// //       <div
// //         className={`relative w-full aspect-square mb-3 sm:mb-4 overflow-hidden rounded-lg `} // use this if you want the background color ${bgClass}`
// //       >
// //         <Image
// //           src={product.img}
// //           alt={product.name}
...
const getProductBgClass = (name: string): string => {
  const upper = name.toUpperCase();
  if (upper.includes("ORIGINAL")) return "bg-[#F9E9D7]";
  if (upper.includes("ICE")) return "bg-[#D7E9F9]";
  if (upper.includes("FIRE")) return "bg-[#F9D7D7]";
  if (upper.includes("GO")) return "bg-[#D7F9E9]";
  if (upper.includes("ENERGIZING")) return "bg-[#D7F9F9]";
  return "bg-gray-50";
};
...
const ProductCard: React.FC<{
  ...
  return (
    <article className="bg-white/90 card shadow-xl p-4 flex flex-col h-[500px] rounded-2xl border border-[#F9461C]/10 hover:shadow-2xl hover:scale-[1.03] transition-all duration-300 relative overflow-hidden group">
      ...
      <div className="relative w-full aspect-square mb-4 overflow-hidden rounded-xl z-10 flex items-center justify-center bg-gray-50">
        <Image
          src={product.img}
          alt={product.name}
          fill
      ...
      <div className="flex flex-col flex-grow z-10">
        <div className="text-[#F9461C] font-extrabold text-sm sm:text-base mb-2 sm:mb-3 min-h-[40px] sm:min-h-[48px] leading-tight">
          {product.name}
        </div>
        {/* <p className="text-gray-700 text-sm line-clamp-3 sm:line-clamp-4 flex-grow">
          {product.desc}
        </p> */}
    ...
    <section className="py-6 sm:py-10">
      <div className="container mx-auto px-4">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 w-full max-w-7xl px-4 z-10 mx-auto">
            {displayProducts.map((p) => (
              <ProductCard
                key={p.id}
                product={p}
...
//                             {t("products.grid.modal.activeIngredient")}
//                           </h3>
//                           <p className="text-gray-800">{tr.activeIngredient}</p>
//                         </div>
//                       )}
//                       {Array.isArray(tr.usage) && tr.usage.length > 0 && (
//                         <div className="bg-[#FFE6B0] p-6 rounded-xl">
//                           <h3 className="font-bold mb-2 text-[#F9461C]">
//                             {t("products.grid.modal.usage")}
//                           </h3>
